class Solution {
    ...
    // Prefix sums to count ones in submatrices
    private int[][] ruc;
    private int height;
    private int width;

    // Returns count of 1s in rectangle [r0, c0) to [r1, c1)
    private int unitsInRectangle(int r0, int c0, int r1, int c1) {
        return ruc[r1][c1] - ruc[r0][c1] - ruc[r1][c0] + ruc[r0][c0];
    }

    // Returns minimum rectangle area enclosing all ones in given submatrix
    ...
    // Find minimum sum of areas when splitting into two rectangles (splitVertical=true means vertical split)
    ...
    // Find minimum sum when splitting into three rectangles using two splits
    // firstSplitVertical: first split orientation
    // takeLower: whether first split takes lower/left part or upper/right part
    // secondSplitVertical: orientation of second split
    ...
    // Main function to find minimum sum of areas of 3 rectangles
    public int minimumSum(int[][] grid) {
        ...
        int min = Integer.MAX_VALUE;
        // Try all combinations of first and second splits orientations and sides
